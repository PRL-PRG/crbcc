interpret <- function(source_dir, target_dir) {
  ...
  for (file_path in csv_files) {
    ...
    # --- Visualization ---
    plot_cyc <- ggplot(df, aes(x = lines_of_code, y = speedup)) +
      ...
      scale_x_log10() +
      labs(
        title = sprintf("Speedup vs Lines of Code: %s", pkg_name),
  ...
  if (length(global_data_list) > 0) {
    ...
    global_geo_mean <- exp(mean(log(global_df$speedup)))
    
    cat("\n---\n\n## Global Benchmark Summary\n\n", file = md_file, append = TRUE)
    cat(sprintf("- **Overall Geometric Average Speedup:** %.4fx\n", global_geo_mean), file = md_file, append = TRUE)
  }
